...
'''
CUDA_VISIBLE_DEVICES=3 python top_n_selection.py \
    --model VGG16 \
    --data_name facescrub \
    --top_n 30 \
    --num_classes 1000
...
parser.add_argument('--save_root', type=str, default='data/reclassified_public_data')
...
class PublicCeleba(torch.utils.data.Dataset):
    def __init__(self, file_path='data/data_files/celeba_ganset.txt',
                 img_root='data/celeba/img_align_celeba', transform=None):
...
def top_n_selection(args, T, data_loader):
    ...
    with torch.no_grad():
        ...
        all_images_prob = torch.cat(all_images_prob)
...
if model_name_T.startswith("VGG16"):
    ...
    T = FaceNet64(1000)
    path_T = 'assets/checkpoints/target_model/aceNet64_88.50.tar'
...
if args.data_name == 'celeba':
    ...
    data_set = PublicCeleba(file_path='data/data_files/celeba_ganset.txt',
                            img_root='data/celeba/img_align_celeba',
                            transform=celeba_transform)
    data_loader = data.DataLoader(data_set, batch_size=1000)
    ...
    data_set = PublicFFHQ(root='data/ffhq/thumbnails128x128/', transform=ffhq_transform)
    data_loader = data.DataLoader(data_set, batch_size=1000)
    ...
    data_set = PublicFaceScrub(file_path='data/data_files/facescrub_ganset.txt',
                               img_root='data/facescrub', transform=faceScrub_transform)
    data_loader = data.DataLoader(data_set, batch_size=1000)
